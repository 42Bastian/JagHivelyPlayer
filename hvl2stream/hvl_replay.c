/*
 * 36 bits
 * 36 bits * 4 = 144 bits = 18 octets par frame
 *
 -track on : 1 bit
 - volume : 6 bits
 - period : 12 bits
 - wavelength : 0 a 5 : 3 bits
 - waveform : 0 a 3 : 2 bits
 - pvt_squarePos : 1 a 64 : 6 bits
 - pvt_filterPos : 1 a 63 : 6 bits
*/
...
typedef struct stream_hd_s {
  uint8_t channels;
  uint8_t multiplier;
  uint8_t panningLeft;
  uint8_t panningRight;
  uint32_t rm_channels;
  uint32_t restartLong;
  uint32_t restartBit;
  uint32_t streamLength;
} stream_hd_t;

int flag_position_restart_lue = 0;

int nombre_de_frames = 0;
int bits_restart = 0;
int nb_bits_ecrits = 0;
const int numero_de_frame_debut_streaming = 0;
uint32_t position_byte32_restart = 0;
uint32_t position_bit_restart = 0;
// 64 * 4 * 1;
// 64*4*10;
// 2750 = 10 ou 11

int nombre_de_waveforms_differentes = 0;
int flag_tracer_fin_de_module = 0;

int taille_totale_stream = 0;
int taille_totale_stream_MAXI = 0;
int taille_totale_stream_flag_bits = 0;
int file_bits_count = 0;
int8 file_bits_buffer = 0;

extern char output_filename[256];

FILE* fd_stream;

int stream_period_max  = 0;
int stream_period_min  = 65536 - 1;

int8 octet_en_cours = 0;
int position_octet_en_cours = 7;

void outputBit(unsigned char bit)
{
  file_bits_buffer <<= 1;         // Make room for next bit.
  if (bit) file_bits_buffer |= 1; // Set if necessary.
  file_bits_count++;              // Remember we have added a bit.
  if (file_bits_count == 8)  {
    fwrite(&file_bits_buffer, sizeof(file_bits_buffer), 1, fd_stream); // Error handling elided.
    file_bits_buffer = 0;
    file_bits_count = 0;
  }
}

void outputByte(unsigned char byte, int nb_bits_dans_byte)
{
  for (int i = 0; i < nb_bits_dans_byte; i++) {
    unsigned char bit = (byte >> ((nb_bits_dans_byte - i) - 1)) & 1;
    outputBit(bit);

  }
  nb_bits_ecrits = nb_bits_ecrits + nb_bits_dans_byte;
}

void close_outputBit()
{
  fwrite(&file_bits_buffer, sizeof(file_bits_buffer), 1, fd_stream);
  file_bits_buffer = 0;
  file_bits_count = 0;
}
...
struct hvl_tune *hvl_LoadTune( const TEXT *name, uint32 freq, uint32 defstereo )
{
  ...
  // EDZ/42BS
  fd_stream = fopen(output_filename, "wb");
  fseek(fd_stream, sizeof(stream_hd_t), SEEK_SET); // skip header
...
void hvl_plist_command_parse( const struct hvl_tune *ht, struct hvl_voice *voice, int32 FX, int32 FXParam )
{
  switch( FX )
    ...
    case 7:
      ...
      // printf("ring modulation with triangle on channel : %d\n", voice->vc_VoiceNum);
      break;
    ...
    case 8:  // Ring modulate with sawtooth
      ...
      voice->vc_RingNewWaveform = 1;
      voice->vc_RingPlantPeriod = 1;
      // printf("ring modulation with sawtooth on channel : %d\n", voice->vc_VoiceNum);
...
void hvl_process_frame( struct hvl_tune *ht, struct hvl_voice *voice )
{
  ...
  // octet1 = track on : 1 bit + volume : 6 bits ( test )

  /*
    track on : vc_TrackOn                       1 bit                   O1
    volume : ht->ht_Voices[i].vc_VoiceVolume    6 bits                  O1
    waveform : vc_Waveform                      2 bits                  O1(1)+O2(1)
    pvt_squarePos : vc_SquarePos                6 bits                  O2
    wavelength : vc_WaveLength                  3 bits                  O2(1)+O3(2)
    vc_FilterPos : vc_FilterPos                6 bits                  O3
    frequence/period : vc_AudioPeriod           12 bits                 O4+O5(4)
    36 bits = 4,5 octets
  */

  if (voice->vc_AudioPeriod > stream_period_max)
    stream_period_max = voice->vc_AudioPeriod;
  if (voice->vc_AudioPeriod < stream_period_min)
    stream_period_min = voice->vc_AudioPeriod;
  ...
  if( voice->vc_Waveform == 3-1 || voice->vc_PlantSquare )
    {
      ...
      // EDZ
      // if ((voice->vc_SquarePos == 0) && (voice->vc_AudioVolume > 0) && (voice->vc_Waveform == 2))
      // {
      //     printf("pos 2 squarepos=0\n");
      // }
      //
  ...
  if( voice->vc_NewWaveform )
    {
      ...
      // vc_Waveform :  0=triangle 1=sawtooth 2=square 3=noise
  ...
  if (voice->vc_VoiceNum == 1)
    {
      int toto = 1;
    }
  // teste si on passe pour la premiere sur la position de restart, alors on prépare le fait d'y revenir, en mettant toutes les old valeurs à -1
  if ((ht->ht_PosNr == ht->ht_Restart) && (flag_position_restart_lue == 0))
    {
      ...
      for (int i = 0; i < MAX_CHANNELS; i++)
        {
          ...
          ht->ht_Voices[i].old_vc_AudioVolume = -1;
          ht->ht_Voices[i].old_vc_Waveform = -1;
          ht->ht_Voices[i].old_vc_SquarePos = -1;
          ht->ht_Voices[i].old_vc_WaveLength = -1;
          ht->ht_Voices[i].old_vc_FilterPos = -1;
          ht->ht_Voices[i].old_vc_AudioPeriod = -1;
          // RM
          ht->ht_Voices[i].old_RM_was_active = 1;
          ht->ht_Voices[i].old_vc_RingAudioPeriod = -1;
          ht->ht_Voices[i].old_vc_RingWaveform = -1;
        }

    }

  // if (voice->vc_TrackOn == 0)
  //     printf("TrackOn sert à quelque chose.\n");
  // if (voice->vc_VoiceNum == 2 && nombre_de_frames == 126)
  // {
  //     printf("zone du bug\n");
  // }
  ...
  int8 octet1 = 0;
  // 	track on : vc_TrackOn                       1 bit                   O1
  octet1 = (voice->vc_TrackOn & 1) << 7;
  //    volume : ht->ht_Voices[i].vc_VoiceVolume    6 bits                  O1
  int tempedz = voice->vc_AudioVolume;
  if (tempedz == 64) tempedz = 63;
  octet1 = octet1 + ((tempedz & 0b111111) << 1);
  //    waveform : vc_Waveform                      2 bits                  O1(1)
  octet1 = octet1 + ((voice->vc_Waveform & 0b10) >> 1);

  int8 octet2 = 0;
  //    waveform : vc_Waveform                      2 bits                  O2(1)
  octet2 = ((voice->vc_Waveform & 0b1) << 7);
  //    pvt_squarePos : vc_SquarePos                6 bits                  O2
  octet2 = octet2 + ((voice->vc_SquarePos & 0b111111) << 1);
  //    wavelength : vc_WaveLength                  3 bits                  O2(1)
  octet2 = octet2 + ((voice->vc_WaveLength & 0b100) >> 2);

  int8 octet3 = 0;
  //    wavelength : vc_WaveLength                  3 bits                  O3(2)
  octet3 = ((voice->vc_WaveLength & 0b11) << 6);
  //    pvt_filterPos : vc_FilterPos                6 bits                  O3
  octet3 = octet3 + ((voice->vc_FilterPos & 0b111111));

  int8 octet4 = 0;
  //    frequence/period : vc_AudioPeriod           12 bits                 O4
  octet4 = ((voice->vc_AudioPeriod & 0b111111110000) >> 4);

  int8 octet5 = 0;
  octet5 = ((voice->vc_AudioPeriod & 0b1111) << 4);

  if ((octet1!=voice->old_octet1) || (octet2!=voice->old_octet2) || (octet3!=voice->old_octet3) || (octet4!=voice->old_octet4) || (octet5!=voice->old_octet5))
    {
      taille_totale_stream = taille_totale_stream + 5;
    }

  // octets pour les bits
  // nb maxi de bits = 7+6+2+6+3+6+12 = 42 bits => 6 octets
  int octet_bits_en_cours = 0;
  int position_dans_octet_bits_en_cours = 0;
  int8 flag_bits_utilises = 0;

  if (voice->old_vc_Pan != voice->vc_Pan)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 8;
      flag_bits_utilises = flag_bits_utilises | 0b10000000;
    }

  if (voice->old_vc_AudioVolume != voice->vc_AudioVolume)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 6;
      flag_bits_utilises = flag_bits_utilises | 0b01000000;
    }

  if (voice->old_vc_Waveform != voice->vc_Waveform)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 2;
      flag_bits_utilises = flag_bits_utilises | 0b00100000;
    }

  if (voice->old_vc_SquarePos != voice->vc_SquarePos)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 6;
      flag_bits_utilises = flag_bits_utilises | 0b00010000;

    }

  if (voice->old_vc_WaveLength != voice->vc_WaveLength)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 3;
      flag_bits_utilises = flag_bits_utilises | 0b00001000;

    }

  if (voice->old_vc_FilterPos != voice->vc_FilterPos)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 6;
      flag_bits_utilises = flag_bits_utilises | 0b00000100;
    }

  if (voice->old_vc_AudioPeriod != voice->vc_AudioPeriod)
    {
      taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 12;
      flag_bits_utilises = flag_bits_utilises | 0b00000010;
    }
  // RM // érroné
  // if (voice->vc_RingNewWaveform)
  // {
  //     taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 17;         // RM : vc_Waveform+vc_WaveLength+vc_AudioPeriod = 2+3+12=17 bits
  //     flag_bits_utilises = flag_bits_utilises | 0b00000001;
  // }

  // panning+6 bits dans tous les cas + RM = 8 bits
  taille_totale_stream_flag_bits = taille_totale_stream_flag_bits + 8;

  taille_totale_stream_MAXI = taille_totale_stream_MAXI + 5;

  // ring modulation
  // si voice->vc_RingMixSource n'est pas NULL, vc_RingNewWaveform n'est pas à zéro

  // test de debug
  if ((voice->vc_VoiceNum == 0) && (voice->vc_RingMixSource == NULL))
    {
      int titi = 1;
    }

  if (voice->vc_RingMixSource != NULL)
    // on a du RM
    {
      if ((voice->old_vc_RingAudioPeriod != voice->vc_RingAudioPeriod) || (voice->vc_RingWaveform != voice->old_vc_RingWaveform))
        {
          voice->old_RM_was_active = 1;
          flag_bits_utilises = flag_bits_utilises | 0b00000001;
        }

    }
  else
    // pas de RM mais le precedent avait du RM
    {
      if (voice->old_RM_was_active == 1)
        {
          voice->old_RM_was_active = 0;
          flag_bits_utilises = flag_bits_utilises | 0b00000001;
  ...
  if ((fd_stream != NULL) && (nombre_de_frames >= numero_de_frame_debut_streaming))
    {
      // fwrite(&octet1, 1, sizeof(octet1), pointeur_fichier_stream);
      // fwrite(&octet2, 1, sizeof(octet2), pointeur_fichier_stream);
      // fwrite(&octet3, 1, sizeof(octet3), pointeur_fichier_stream);
      // fwrite(&octet4, 1, sizeof(octet4), pointeur_fichier_stream);
      // fwrite(&octet5, 1, sizeof(octet5), pointeur_fichier_stream);
      ...
      // }
      //
      // if ((nombre_de_frames == 315) && (voice->vc_VoiceNum==2))           // channel 3
      // {
      //     printf("toto\n");
      // }

      // fichier bits / flag de debut
      outputByte(flag_bits_utilises, 8);

      // fwrite(&flag_bits_utilises, 1, sizeof(flag_bits_utilises), fd_stream);

      if (voice->old_vc_Pan != voice->vc_Pan)
        {
          outputByte(voice->vc_Pan, 8);

        }

      if (voice->old_vc_AudioVolume != voice->vc_AudioVolume)
        {
          //->          if (voice->vc_AudioVolume >= 64) printf("volume satur\n");

          if (voice->vc_AudioVolume >= 64) voice->vc_AudioVolume = 64 - 1;
          outputByte(voice->vc_AudioVolume, 6);

        }

      if (voice->old_vc_Waveform != voice->vc_Waveform)
        {
          outputByte(voice->vc_Waveform, 2);
        }

      if (voice->old_vc_SquarePos != voice->vc_SquarePos)
        {
          outputByte(voice->vc_SquarePos, 6);

        }
      if (voice->old_vc_WaveLength != voice->vc_WaveLength)
        {
          outputByte(voice->vc_WaveLength, 3);

        }
      if (voice->old_vc_FilterPos != voice->vc_FilterPos)
        {
          outputByte(voice->vc_FilterPos, 6);
        }
      if (voice->old_vc_AudioPeriod != voice->vc_AudioPeriod)
        {
          outputByte((voice->vc_AudioPeriod>>8), 4);
          outputByte(voice->vc_AudioPeriod , 8);
        }

      if (flag_bits_utilises & 1)
        {
          // RM a ecrire
          if (voice->old_RM_was_active == 1)
            {
              outputByte(voice->vc_RingWaveform, 1);
              outputByte((voice->vc_RingAudioPeriod >> 8), 4);
              outputByte(voice->vc_RingAudioPeriod, 8);
              voice->RM_was_used++;
            }
          else
            {
      ...
      voice->old_octet1 = octet1;
      voice->old_octet2 = octet2;
      voice->old_octet3 = octet3;
      voice->old_octet4 = octet4;
      voice->old_octet5 = octet5;

      voice->old_vc_Pan = voice->vc_Pan;
      voice->old_vc_AudioVolume = voice->vc_AudioVolume;
      voice->old_vc_Waveform = voice->vc_Waveform;
      voice->old_vc_SquarePos = voice->vc_SquarePos;
      voice->old_vc_WaveLength = voice->vc_WaveLength;
      voice->old_vc_FilterPos = voice->vc_FilterPos;
      voice->old_vc_AudioPeriod = voice->vc_AudioPeriod;
    }
}
...
void hvl_set_audio( struct hvl_voice *voice, float64 freqf )
{
  ...
  if( voice->vc_NewWaveform )
    {
      const int8 *src;
      nombre_de_waveforms_differentes++;
...
void hvl_play_irq( struct hvl_tune *ht )
{
  ...
  // printf("  %d / %d\r",ht->ht_PosNr,ht->ht_PositionNr);
  ...
  if( --ht->ht_StepWaitFrames == 0 )
    {
      ...
      // EDZ
      flag_tracer_fin_de_module = (ht->ht_PosNr > 1 && flag_tracer_fin_de_module == 0);
      ...
      if( ht->ht_PatternBreak )    {
          ...
          if ( ( ht->ht_PosNr == ht->ht_PositionNr ) ||
               (ht->ht_PosNr < 2 && flag_tracer_fin_de_module == 1))
            {
              ht->ht_SongEndReached = 1;
              ht->ht_PosNr          = ht->ht_Restart;

              nombre_de_frames = 0;
              nombre_de_waveforms_differentes = 0;
              ...
              if  (fd_stream != NULL)
                {
                  ...
                  for (i = 0; i < ht->ht_Channels; i++){
                    if (ht->ht_Voices[i].RM_was_used > 0)  {
                      rm_mask |= (1<<i);
                    }
                  }
                  size = ftell(fd_stream) - sizeof(stream_hd_t);
                  /* align size on 4 bytes */
                  if ( size & 3 ){
                    uint32_t null = 0;
                    fwrite(&null, 4-(size & 3), 1, fd_stream);
                    size += 4-(size & 3);
                  }
                  ...
                  ** Write header
                  */
                  fseek(fd_stream, 0, SEEK_SET);
                  fwrite( &ht->ht_Channels       ,1,1,fd_stream);
                  fwrite( &ht->ht_SpeedMultiplier,1,1,fd_stream);
                  fwrite( &ht->ht_defpanleft     ,1,1,fd_stream);
                  fwrite( &ht->ht_defpanright    ,1,1,fd_stream);

                  c = rm_mask >> 24; fwrite( &c,1,1,fd_stream);
                  c = rm_mask >> 16; fwrite( &c,1,1,fd_stream);
                  c = rm_mask >> 8;  fwrite( &c,1,1,fd_stream);
                  c = rm_mask;       fwrite( &c,1,1,fd_stream);

                  c = position_byte32_restart >> 24; fwrite( &c,1,1,fd_stream);
                  c = position_byte32_restart >> 16; fwrite( &c,1,1,fd_stream);
                  c = position_byte32_restart >> 8;  fwrite( &c,1,1,fd_stream);
                  c = position_byte32_restart;       fwrite( &c,1,1,fd_stream);

                  c = position_bit_restart >> 24; fwrite( &c,1,1,fd_stream);
                  c = position_bit_restart >> 16; fwrite( &c,1,1,fd_stream);
                  c = position_bit_restart >> 8;  fwrite( &c,1,1,fd_stream);
                  c = position_bit_restart;       fwrite( &c,1,1,fd_stream);

                  c = size >> 24; fwrite( &c   ,1,1,fd_stream);
                  c = size >> 16; fwrite( &c   ,1,1,fd_stream);
                  c = size >> 8;  fwrite( &c   ,1,1,fd_stream);
                  c = size;       fwrite( &c   ,1,1,fd_stream);
                  fclose(fd_stream);
                  ...
                  printf("speed mult         : %d\n", ht->ht_SpeedMultiplier);
                  printf("ht_defpanleft      : %d\n", ht->ht_defpanleft);
                  printf("ht_defpanright     : %d\n", ht->ht_defpanright);
                  printf("Bits written       : %d\n", nb_bits_ecrits);
                  printf("restart longword   : %d\n", position_byte32_restart);
                  printf("restart bit        : %d\n", position_bit_restart );
                  printf("RM channels        : %08x\n", rm_mask);
                  fd_stream = NULL;

                  if (stream_period_max > (1 << 12)){
                    printf("--------ERROR : period too high !!!!--------\n");
                  }
                  printf("\n");
                  hvl_FreeTune( ht );
                  exit(0);
...
void hvl_DecodeFrame( struct hvl_tune *ht )
{
